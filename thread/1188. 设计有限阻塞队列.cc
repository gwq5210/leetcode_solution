 ...
 * [1188] 设计有限阻塞队列
 *
 * https://leetcode.cn/problems/design-bounded-blocking-queue/description/
...
由于生产者/消费者线程的数目可能大于 1
，我们并不知道线程如何被操作系统调度，即使输入看上去隐含了顺序。因此任意一种输出[1,0,2]或[1,2,0]或[0,1,2]或[0,2,1]或[2,0,1]或[2,1,0]都可被接受。
...
class CVBoundedBlockingQueue {
 public:
  CVBoundedBlockingQueue(int capacity) : capacity_(capacity) {}

  void enqueue(int element) {
    std::unique_lock<std::mutex> ul(lock_);
    cv_.wait(ul, [this]() { return q_.size() < capacity_; });
    q_.push(element);
    cv_.notify_all();
  }
  ...
  int dequeue() {
    std::unique_lock<std::mutex> ul(lock_);
    cv_.wait(ul, [this]() { return q_.size() > 0; });
    int front = q_.front();
    q_.pop();
    cv_.notify_all();
    return front;
...
class SemBoundedBlockingQueue {
 public:
  SemBoundedBlockingQueue(int capacity) : capacity_(capacity), in_sem_(capacity), out_sem_(0) {}
 ...
 private:
  int capacity_ = 0;
  std::counting_semaphore in_sem_;
  std::counting_semaphore out_sem_;
  std::queue<int> q_;
  std::mutex lock_;
};

class CSemBoundedBlockingQueue {
 public:
  CSemBoundedBlockingQueue(int capacity) : capacity_(capacity) {
    sem_init(&in_sem_, 0, capacity_);
    sem_init(&out_sem_, 0, 0);
  }
