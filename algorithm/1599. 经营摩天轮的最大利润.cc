 ...
 * 逆时针 轮转座舱，但每次轮转都需要支付一定的运行成本 runningCost 。摩天轮每次轮转都恰好转动 1 / 4 周。
 *
 ...
 * 。如果你决定停止运营摩天轮，为了保证所有游客安全着陆，将免费进行所有后续轮转 。注意，如果有超过 4
 ...
 * 2. 9 位游客抵达，4 位登舱，11 位等待（2 位是先前就在等待的，9 位新加入等待的），摩天轮轮转。当前利润为 8 * $6 - 2 *
 * $4 = $40 。
...
class Solution {
 public:
  int minOperationsMaxProfit(std::vector<int>& customers, int boardingCost, int runningCost) {
    ...
    int prev = 0;
    int curr_in = 0;
    // 模拟，前 n 轮需要计算游客来的数量，后续则计算等待的游客数量
    // 后一阶段可以直接通过除法得到结果
    for (int i = 0; i < customers.size() || prev; ++i) {
      int c = customers[i];
      int curr = c + prev;
      int x = 0;
      if (curr >= 4) {
      ...
      curr_in += x * boardingCost - runningCost;
      int y = curr_in;
      if (curr_in > res) {
        res = curr_in;
        res_i = i;
