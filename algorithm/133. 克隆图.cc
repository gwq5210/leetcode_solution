...
class Solution {
 public:
  Solution() : nodes_(101) {}
  Node* cloneGraph(Node* node) { return cloneGraphDFS(node); }
  Node* cloneGraphDFS(Node* node) {
    if (node == nullptr) {
      return nullptr;
    }

    if (nodes_[node->val] != nullptr) {
      return nodes_[node->val];
    }
    nodes_[node->val] = new Node(node->val);

    for (Node* neighbor : node->neighbors) {
      nodes_[node->val]->neighbors.emplace_back(cloneGraphDFS(neighbor));
    }
    return nodes_[node->val];
  }
  Node* cloneGraphBFS(Node* node) {
    ...
    while (!q.empty()) {
      ...
      for (Node* v : front->neighbors) {
        if (nodes_[v->val] == nullptr) {
          q.push(v);
          nodes_[v->val] = new Node(v->val);
        }
        nodes_[front->val]->neighbors.emplace_back(nodes_[v->val]);
      }
    }
    return nodes_[node->val];
  }
